...
def user_subscription_view(request):
    """
    Display user's subscription view with details
    """
    profile = UserProfile.objects.get(user=request.user)

    if not profile.subscription:
        messages.error(
            request, "You haven't subscrtibed yet. Please choose one and join the Finesse Fitness Family")
        return redirect(reverse('subscriptions'))

    subscription = get_object_or_404(Subscription, name=profile.subscriptions)
    context = {
        'subscription': subscription
    }
    template = 'subscription/user_subscription.html'
...
def subscription_update(request):
    ...
    try:
        ...
        subscription_type = get_object_or_404(Subscription, name=subscriptions)
